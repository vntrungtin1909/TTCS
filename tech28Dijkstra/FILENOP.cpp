...
{
protected:
    int n, m, batdau;      // số lượng đỉnh và số lượng cạnh
    int pre[maxn]; // mảng lưu đường đi đến đích

public:
    Dijkstra(const GraphNhapXuat &input) :  n(input.so_luong_dinh), m(input.so_luong_canh), batdau(input.dinh_batdau) {}
    ...
    virtual void NganNhat(Graph &graph)
    {
        ...
        while (a[0].dinh != INF)
        {
            ...
            while (1)
            {
                if (a[i].dinh == INF && a[i].trong_so == INF)
                    break;
                else
                {
                    a[i].dinh = a[i + 1].dinh;
...
protected:
    ...
    int pre[maxn];
public:
    Astar(const GraphNhapXuat &input) :  n(input.so_luong_dinh), m(input.so_luong_canh), batdau(input.dinh_batdau), dich(input.dinh_dich) {}
...
int main()
{
    ...
    GraphNhapXuat userInput("input1.txt", "output4124.txt");
